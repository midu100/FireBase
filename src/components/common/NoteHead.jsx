...
const NoteHead = ({editNoteData}) => {
      const db = getDatabase();
  
  // reduxData
  const currentUserData = useSelector((state)=>state.currentUser.value)
  console.log(currentUserData)
  const currentEditData = useSelector((state)=>state.currentUser.editValue)
...
useEffect(()=>{
    if(currentEditData){
        setIsUpdate(true)
        setInputValue(currentEditData?.notes?.noteHead)
        setNoteContent(currentEditData?.notes?.noteContent)
    }
    else{
        setIsUpdate(false)
        setInputValue('')
...
// edit Update
const handleUpdate = ()=>{
    update(ref(db , 'allNotes/' + currentEditData.Key) , {
        noteHead : inputValue,
        noteContent: noteContent,
        noteColor : color,
    })
    ...
    <>
            <section  className="max-w-4xl mx-auto px-4  py-[10px] ">
              ...
              <div  className={`bg-[${color}] rounded-lg shadow p-4 flex justify-between items-center space-x-3`}>
                ...
                {
                    isUpdate ?
                    <button onClick={handleUpdate} className="bg-yellow-500 hover:bg-yellow-600 text-white p-2 rounded-full">
                        <GrUpdate />
                    </button>
